...
class WordsViewState extends State<WordsView> {
  ...
  Widget build(BuildContext context) {
    // print(widget.vocabulary.getTypeSlug);
    return Scaffold(
      appBar: AppBar(
                  ...
                  child: TextFormField(
                    ...
                    validator: (text) {
                      ...
                      if (Validator.checkTextLength(text!)){
                        return 'Допускается только длина ${_config.getMaxAvailableWordLength} символов';
                      }
                      if (widget.vocabulary.getTypeSlug == 'rus_en') {
                        return Validator.checkLanguage(text, _latinRegExp)
                            ? 'Не допускается латиница'
                            : null;
                      }
                      if (widget.vocabulary.getTypeSlug == 'en_rus') {
                ...
                Form(
                    ...
                    child: TextFormField(
                      ...
                      validator: (text) {
                        if (Validator.checkIsTextIsEmpty(text)) {
                          return 'Поле не должно быть пустым';
                        } else if (Validator.checkTranslateLongPhraseLength(text!)){
                        return 'Допускается только длина ${_config.getMaxAvailableTranslateLongPhraseLength} символов';
                        } else if (widget.vocabulary.getTypeSlug == 'en_rus') {
                          return Validator.checkLanguage(text, _latinRegExp)
                              ? 'Латиница не допускается'
                              : null;
                        } else if (widget.vocabulary.getTypeSlug == 'rus_en') {
                          return Validator.checkLanguage(text, _cyrillicRegExp)
